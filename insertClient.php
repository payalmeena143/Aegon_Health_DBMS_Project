...
<?php
		...
		// Check if image file is a act
			...
			if($check !== false) {
				echo "Clients profile picture uploaded- " . $check["mime"] . "."; echo '</br>';
				$uploadOk = 1;
			} else {
	...
	$sql = "INSERT INTO client "."VALUES('$client_id', '$client_password', '$name', '$sex', '$birth_date', '$nid', '$phone', '$address', '$policy_id', '$renewal_date', '$agent_id','$profileImage')";
	...
	if ($conn->query($sql) === true) {
		...
		/// NOMINEE BEING ADDED
   $sql = "INSERT INTO claims "."VALUES('$claim_id', '$claim_date', '$client_id', '$policy_id', '$nid', '$disease_name', '$hospital_name', '$status', '$reason')";
	
	if ($conn->query($sql) === true) {
			echo "New Claim ADDED";  echo '</br>';
		} else {
			echo "Error: " . $sql . "<br>" . $conn->error;  echo '</br>';
		}
